...
def match_functions_by_selector(
    ...
    if analysis_quality_reject_reasons(analyzer):
        return []
    sol_candidates = defaultdict(list)
    bc_candidates = defaultdict(list)
    for function in solidity_functions:
        if function.get("selector"):
            sol_candidates[function["selector"]].append(function)
    for function in bytecode_functions.values():
        if getattr(function, "selector", None):
            bc_candidates[function.selector].append(function)
    # Selector collisions without resolved source identity are not labels.
    sol_by_sel = {key: values[0] for key, values in sol_candidates.items() if len(values) == 1}
    bc_by_sel = {key: values[0] for key, values in bc_candidates.items() if len(values) == 1}
