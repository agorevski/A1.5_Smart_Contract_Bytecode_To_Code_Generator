...
class BytecodeSemanticResult:
    """Source-fact proxy with reference-bytecode hints and optional runtime checks.

    ``checked`` means reference bytecode/opcode evidence is present, not that
    generated bytecode was compared or that behavior was verified.
    ``runtime_bytecode_match`` means exact full-contract bytecode identity,
    not semantic or deployment equivalence.
    """
    ...
    runtime_bytecode_skip_reason: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "score": self.score,
            "score_kind": "source_fact_overlap_proxy",
            ...
            "runtime_bytecode_skip_reason": self.runtime_bytecode_skip_reason,
            "runtime_comparison_kind": (
                "exact_full_contract_runtime_bytecode" if self.runtime_bytecode_checked else None
            ),
...
def evaluate_bytecode_semantics(
    ...
    *,
    ...
    """Score source-fact overlap with available reference bytecode/TAC diagnostics.
    ...
    runtime_match, runtime_skip_reason = _compare_grounded_runtime_bytecode(
        reference_code, candidate_code, metadata
    )
    runtime_checked = runtime_match is not None
    if runtime_match is not None:
        if not runtime_match:
            mismatch_buckets["runtime_bytecode_mismatch"].append("compiled_runtime_differs")
    ...
    checked = bool(
        _first_bytecode_value(metadata)
        or _extract_result_opcodes({"metadata": metadata})
        or opcode_slices["opcode_groups"]
    )
    if not checked:
        score = 0.0
    ...
    return BytecodeSemanticResult(
        ...
        runtime_bytecode_skip_reason=runtime_skip_reason,
...
def _exact_evm_bytecode(bytecode: Any) -> Optional[str]:
    """Accept complete hex bytecode, without silently dropping non-hex characters."""
    if not isinstance(bytecode, str):
        return None
    text = bytecode.removeprefix("0x").removeprefix("0X")
    if not text or len(text) % 2 or re.fullmatch(r"[0-9a-fA-F]+", text) is None:
        return None
    return text.lower()


def _compare_grounded_runtime_bytecode(
    reference_code: str, candidate_code: str, metadata: Mapping[str, Any]
) -> Tuple[Optional[bool], Optional[str]]:
    """Compare named, self-contained contracts only when their reference runtime is verified.

    ``runtime_comparison`` opts in with contract_name, compiler_version,
    optimizer_enabled, optimizer_runs, and optionally source_name (default:
    contract.sol, the local dataset compiler's source unit). The metadata
    ``runtime_bytecode``/``deployed_bytecode`` or ``bytecode`` must reproduce
    from the complete reference source with those exact settings. No creation
    bytecode, provided candidate runtime, or fragment harness is accepted.
    Returns exact bytecode equality and a per-row reason when unchecked.
    """
    if "runtime_comparison" not in metadata:
        return _runtime_comparison_skipped("not_opted_in")
    settings = metadata["runtime_comparison"]
    if not isinstance(settings, Mapping):
        return _runtime_comparison_skipped("invalid_compiler_settings")
    contract_name = settings.get("contract_name")
    version = settings.get("compiler_version")
    optimized = settings.get("optimizer_enabled")
    runs = settings.get("optimizer_runs")
    source_name = settings.get("source_name", "contract.sol")
    if (
        not isinstance(contract_name, str)
        or re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", contract_name) is None
        or not isinstance(version, str)
        or re.fullmatch(r"\d+\.\d+\.\d+", version) is None
        or type(optimized) is not bool
        or type(runs) is not int
        or runs < 0
        or not isinstance(source_name, str)
        or re.fullmatch(r"[A-Za-z_][A-Za-z0-9_.-]*\.sol", source_name) is None
    ):
        return _runtime_comparison_skipped("invalid_compiler_settings")
    if (
        (_metadata_compiler_version(metadata) not in (None, version))
        or ("optimizer_enabled" in metadata and metadata["optimizer_enabled"] is not optimized)
        or ("optimizer_runs" in metadata and metadata["optimizer_runs"] != runs)
    ):
        return _runtime_comparison_skipped("conflicting_compiler_settings")
    expected = _first_bytecode_value(metadata, ("runtime_bytecode", "deployed_bytecode"))
    if not expected:
        expected = metadata.get("bytecode")
    expected_hex = _exact_evm_bytecode(expected)
    if expected_hex is None:
        return _runtime_comparison_skipped("missing_or_invalid_reference_runtime")

    reference_runtime, reference_reason = _compile_named_runtime(
        reference_code, contract_name, version, optimized, runs, source_name
    )
    if reference_runtime is None:
        return _runtime_comparison_skipped(f"reference_{reference_reason}")
    if reference_runtime != expected_hex:
        return _runtime_comparison_skipped("reference_runtime_mismatch")
    candidate_runtime, candidate_reason = _compile_named_runtime(
        candidate_code, contract_name, version, optimized, runs, source_name
    )
    if candidate_runtime is None:
        return _runtime_comparison_skipped(f"candidate_{candidate_reason}")
    return candidate_runtime == expected_hex, None


def _runtime_comparison_skipped(reason: str) -> Tuple[None, str]:
    if reason != "not_opted_in":
        logger.info("Exact runtime bytecode comparison skipped: %s", reason)
    return None, reason


def _compile_named_runtime(
    source_code: str,
    contract_name: str,
    version: str,
    optimized: bool,
    ...
    """Compile a full, constructor-free contract without installing solc."""
    masked_source = _mask_solidity_comments_and_strings(source_code or "")
    if re.search(r"\bimport\s", masked_source):
        return None, "imports_unsupported"
    if not re.search(rf"\bcontract\s+{re.escape(contract_name)}\b", masked_source):
        reason = (
            "target_contract_missing"
            if re.search(r"\bcontract\s+[A-Za-z_][A-Za-z0-9_]*\b", masked_source)
            else "fragment"
        )
        return None, reason
    try:
        import solcx
        from solcx.exceptions import SolcError, SolcInstallationError, SolcNotInstalled
    except ImportError:
        return None, "solcx_unavailable"
    except OSError as exc:
        logger.warning("Local solcx import failed for runtime comparison: %s", exc)
        return None, "solcx_import_io_error"

    try:
        if version not in {str(installed) for installed in solcx.get_installed_solc_versions()}:
            return None, "solc_version_not_installed"
    except OSError as exc:
        logger.warning("Local solc discovery failed for runtime comparison: %s", exc)
        return None, "compiler_discovery_io_error"
    ...
    try:
        output = solcx.compile_standard(
            {
                ...
                "settings": {
                    "optimizer": {"enabled": optimized, "runs": runs},
                    "outputSelection": {
                        "*": {
                            "": ["ast"],
        ...
        return None, "compiler_rejected_source"
    except (SolcNotInstalled, SolcInstallationError):
        return None, "compiler_unavailable"
    except OSError as exc:
        logger.warning("Local solc failed for runtime comparison: %s", exc)
        return None, "compiler_io_error"

    ast = output.get("sources", {}).get(source_name, {}).get("ast", {})
    contracts = [
        node
        for node in ast.get("nodes", [])
        if node.get("nodeType") == "ContractDefinition" and node.get("name") == contract_name
    ]
    if len(contracts) != 1 or contracts[0].get("contractKind") != "contract":
        return None, "target_contract_missing"
    contract = contracts[0]
    if contract.get("abstract") or contract.get("baseContracts"):
        return None, "abstract_or_inherited_contract"
    for node in contract.get("nodes", []):
        if node.get("nodeType") == "FunctionDefinition" and node.get("kind") == "constructor":
            return None, "constructor_or_state_initializer"
        if node.get("nodeType") == "VariableDeclaration" and node.get("stateVariable"):
            if node.get("value") is not None or node.get("mutability") == "immutable":
                return None, "constructor_or_state_initializer"
    runtime = (
    ...
    compiled = _exact_evm_bytecode(runtime)
    return (compiled, None) if compiled is not None else (None, "compiled_runtime_unavailable")
...
def _first_bytecode_value(
    ...
    for nested_key in ("evm", "metadata", "compiler_output", "bytecode_metadata"):
        ...
        if direct:
            return direct
        if "runtime_bytecode" in keys or "deployed_bytecode" in keys:
...
class SmartContractTrainingPipeline:
    ...
    def _compute_aggregate_statistics(
        ...
        runtime_checked_n = sum(
            result["metrics"].get("bytecode_runtime_checked") is True for result in results
        )
        runtime_equal_n = sum(
            result["metrics"].get("bytecode_runtime_checked") is True
            and result["metrics"].get("bytecode_runtime_match") is True
            for result in results
        )
        runtime_skip_reasons: Counter[str] = Counter()
        for result in results:
            metrics = result["metrics"]
            details = metrics.get("metadata")
            if metrics.get("bytecode_runtime_checked") is True or not isinstance(details, Mapping):
                continue
            bytecode_details = details.get("bytecode_semantics")
            if isinstance(bytecode_details, Mapping):
                reason = bytecode_details.get("runtime_bytecode_skip_reason")
                if isinstance(reason, str) and reason:
                    runtime_skip_reasons[reason] += 1
        stats["runtime_bytecode_comparison"] = {
            "checked_n": runtime_checked_n,
            "equal_n": runtime_equal_n,
            "total_n": len(results),
            "equality_rate_checked": (
                runtime_equal_n / runtime_checked_n if runtime_checked_n else None
            ),
