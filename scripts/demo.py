...
def demo_model_setup():
    """Demonstrate the default Qwen2.5-Coder model configuration with LoRA.
    ...
    """
    print("\n" + "=" * 60)
    print("DEMO 3: Qwen2.5-Coder Model Setup with LoRA")
    print("=" * 60)

    try:
        # Create model configuration matching the default training recipe
        ...
        print("✓ Configuration matches the default training recipe")

        # Show what would happen during actual setup
        print(f"\nModel Setup Process (simulated):")
        print("1. Load Qwen2.5-Coder tokenizer")
        print("2. Load base model in BF16/FP16 when CUDA is available")
        ...
        print("5. Print trainable parameters summary")
...
function changeOwner:
  ...
  storage[0x0] = newOwner""",
    ...
    print(
        f"- Average TAC length: {sum(len(e['input']) for e in sample_entries) / len(sample_entries):.0f} chars"
    )
    print(
        f"- Average Solidity length: {sum(len(e['output']) for e in sample_entries) / len(sample_entries):.0f} chars"
    )
    print(
        f"- Functions with view/pure: {sum(1 for e in sample_entries if e['metadata']['is_view'])}"
    )
    print(
        f"- Functions with payable: {sum(1 for e in sample_entries if e['metadata']['is_payable'])}"
    )
...
def demo_paper_metrics():
    ...
    simulated_results = {
        ...
        "edit_distance": {"average": 0.3, "below_0_4": 0.825},  # 82.5% below 0.4
        "code_length": {
            "correlation": 0.89,
            "median_difference": 5,
    ...
    print(
        f"- Functions > 0.8 similarity: {simulated_results['semantic_similarity']['above_0_8']:.1%}"
    )
    print(
        f"- Functions > 0.9 similarity: {simulated_results['semantic_similarity']['above_0_9']:.1%}"
    )
    ...
    print(
        f"- Median length difference: {simulated_results['code_length']['median_difference']} characters"
    )
    ...
    print(f"\nComparison with Traditional Decompilers:")
    print(
        f"- Traditional decompilers typically achieve 40-50% functions above 0.8 semantic similarity"
    )
...
def main():
    ...
    print("Implementation of Qwen2.5-Coder LoRA fine-tuning for EVM bytecode decompilation")
    ...
    if successful_demos == total_demos:
        ...
        print("2. Model training with Qwen2.5-Coder + LoRA")
